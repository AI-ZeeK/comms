...
namespace Comms.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        public ChatController(CommunicationsDbContext context, ILogger<ChatController> logger, IRabbitMQService rabbitMQService)
        {
            _context = context;
            _logger = logger;
            ...
            try
            {
                var chats = await _context.ChatParticipants
                    ...
                    .Select(cp => new
                    {
                        ...
                        name = cp.Chat.Name,
                        avatarUrl = cp.Chat.AvatarUrl,
                        chatType = cp.Chat.ChatType.ToString(),
                        lastMessage = cp.Chat.Messages.FirstOrDefault(),
                        participantCount = cp.Chat.Participants.Count(p => p.IsActive),
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetChatMessages(Guid chatId, [FromQuery] int page = 1, [FromQuery] int pageSize = 50)
        {
            try
            {
                ...
                var isParticipant = await _context.ChatParticipants
                    .AnyAsync(cp => cp.ChatId == chatId && cp.UserId == userId && cp.IsActive);
        ...
        public async Task<ActionResult<object>> CreateChat([FromBody] CreateChatRequest request)
        {
            try
            {
                ...
                await _rabbitMQService.PublishChatEventAsync("chat.created", new
                {
                    chatId = chat.ChatId,
                    name = chat.Name,
                ...
                return CreatedAtAction(nameof(GetUserChats), new { userId = userId.Value }, new
                {
                    chatId = chat.ChatId,
                    name = chat.Name,
                    chatType = chat.ChatType.ToString(),
