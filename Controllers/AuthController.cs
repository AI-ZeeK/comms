...
namespace Comms.Controllers
{
    ...
    public class AuthController(ILogger<AuthController> logger, CommunicationsDbContext context) : ControllerBase
    {
        ...
        [AllowAnonymous]
        public async Task<IActionResult> SavePushSubscription([FromBody] PushSubscriptionDto request)
        {
            try
            {
                _logger.LogInformation("Saving push subscription for user {UserId}", request.UserId);
            ...
            {
                _logger.LogError(ex, "Error saving push subscription for user {UserId}", request.UserId);
