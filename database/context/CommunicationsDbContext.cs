...
namespace Comms.Data
{
    public class CommunicationsDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<UnreadMessageCount>().HasKey(umc => new { umc.UserId, umc.ChatId });
            ...
            modelBuilder.Entity<VapidKeys>().HasIndex(vk => vk.PublicKey).IsUnique();

            modelBuilder.Entity<MetaData>().HasIndex(vk => vk.Id).IsUnique();
            ...
            modelBuilder.Entity<MessageRead>().HasIndex(mr => mr.UserId);

            modelBuilder.Entity<UnreadMessageCount>().HasIndex(umc => umc.UserId);

            modelBuilder.Entity<PushSubscription>().HasIndex(ps => ps.UserId);

            modelBuilder.Entity<VapidKeys>().HasIndex(vk => vk.PublicKey);
            ...
            modelBuilder.Entity<Message>().Property(m => m.MediaUrls).HasColumnType("text[]");

            // Configure enums to be stored as strings
            modelBuilder.Entity<Chat>().Property(c => c.ChatType).HasConversion<string>();

            modelBuilder.Entity<Chat>().Property(c => c.Status).HasConversion<string>();

            modelBuilder.Entity<Message>().Property(m => m.Type).HasConversion<string>();

            modelBuilder.Entity<Message>().Property(m => m.Status).HasConversion<string>();
        }
